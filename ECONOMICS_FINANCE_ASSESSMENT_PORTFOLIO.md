# 10-option items in five applied domains

Self-directed question writing in market microstructure, macroprudential policy, behavioral finance / experimental economics, urban economics, and tokenomics / DeFi.

Each accepted item has one key, nine expert-plausible distractors, a Medium / Hard / Expert label, a written derivation, and 1–5 references that support the keyed claim. Not a desk, a CFA, or a protocol job.

The identification and methods labs (DiD, TVM, coverage, scoring fluent answers) are listed in [ECONOMICS_EXPERT_PORTFOLIO.md](ECONOMICS_EXPERT_PORTFOLIO.md). They are methods. They are not substitutes for the items below.

## Corpus

**[economics-finance-assessment-benchmark-lab](https://github.com/pavanamthomas/economics-finance-assessment-benchmark-lab)**

40 accepted items (eight in each domain) and 11 rejected drafts. CI checks schema, option count, uniqueness write-up, citation count, and numerical blocks. It does not certify that the economics is true. That residual is the derivation, `MANUAL_REVIEW_CHECKLIST.md`, and whoever reads them.
...
| Domain | In the corpus | Executable lab |
| --- | --- | --- |
...
[quantitative-finance-models](https://github.com/pavanamthomas/quantitative-finance-models) is valuation and risk summaries, not a limit-order book.

### Macroprudential policy

Flagship: `MP-E-01` (Nash fire sale versus coordinated freeze). Three identical banks, shock `p = 0.92`. Freeze κ **9.09%**. Restore a 10% target: the whole float is sold, `p = 0.276`, terminal κ **6.86%**. Releasing the CCyB is the freeze in that calibration. [FLAGSHIP_CASE_STUDY.md](https://github.com/pavanamthomas/macroprudential-systemic-risk-lab/blob/main/FLAGSHIP_CASE_STUDY.md)

### Behavioral finance

Flagship: `BF-E-01` — a CPT prediction is not unique until the reference point is stated. The lab prints the TK92 table: purchase R=100 takes the 50-50 (gap **+0.44**); spot R=80 locks (gap **−4.74**); linear `v(x)=x` is indifferent at every R. [FLAGSHIP_CASE_STUDY.md](https://github.com/pavanamthomas/behavioral-finance-experimental-economics-lab/blob/main/FLAGSHIP_CASE_STUDY.md)
...
Flagship: `UE-H-01` — closed-city versus open-city amenity shock. The lab: closed dR ≈ **+0.094** with du > 0; open dR ≈ **+0.361** with du = 0 and N 1 → 1.555. “Rents rise and residents are better off in both models” is false. [FLAGSHIP_CASE_STUDY.md](https://github.com/pavanamthomas/urban-economics-spatial-equilibrium-lab/blob/main/FLAGSHIP_CASE_STUDY.md)

The DiD / IV / RD lab is not a bid-rent envelope or an open-city pin.

### Tokenomics

Flagship: `TD-E-01` (the three prices). The lab adds the 30 bp fee case that breaks the geometric-mean identity, IL versus hold, arb restoration, health factor, stale oracles, and ordering rent on a constructed pool — not a bot. [FLAGSHIP_CASE_STUDY.md](https://github.com/pavanamthomas/tokenomics-defi-mechanism-design-lab/blob/main/FLAGSHIP_CASE_STUDY.md)

## Authoring versus scoring answers

`ai-response-evaluation-benchmarks` scores *answers*. This laboratory writes the *questions*, including the ones that were rejected.
...
No implied employment at a venue, a central bank, or a protocol. No live returns, no AUM, no client tape, no mainnet deployment.
